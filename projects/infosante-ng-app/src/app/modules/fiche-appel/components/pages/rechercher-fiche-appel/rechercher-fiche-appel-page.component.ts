...
const CRITERE_RECHERCHE_STORAGE: string = "critereRechercheAppelSA";
/** "SA" */
...
export class RechercherFicheAppelPageComponent implements OnInit, OnDestroy {
  ...
  ngOnInit(): void {
    ...
    this.subscriptions.add(
      forkJoin([
        ...
        this.referencesApiService.getListeIntervenantOrganismeWithRole(idStOrganismeCourant, "SA_APPEL_MODIF", true).pipe(catchError(e => of(null))), // 4
        ...
        this.referencesApiService.getListeNiveauUrgence(true).pipe(catchError(e => of(null))),// 7

        this.referencesUsagerApiService.getListeLangue(true).pipe(catchError(e => of(null))), // 8
        this.referencesUsagerApiService.getListeRegion(true).pipe(catchError(e => of(null))), // 9
        this.referencesUsagerApiService.getListeSexe(true).pipe(catchError(e => of(null))),   // 10

        this.referencesApiService.getListeCategorieAppelant(true).pipe(catchError(e => of(null))),  // 11

        this.referencesApiService.getListeTypeNote(true).pipe(catchError(e => of(null))),           // 12
        this.referencesApiService.getListeIntervenantOrganismeWithRole(idStOrganismeCourant, "SA_APPEL_NOTE", true).pipe(catchError(e => of(null))), // 13
        this.referencesApiService.getListeCentreActivite().pipe(catchError(e => of(null))),         // 14

        this.referencesUsagerApiService.getListeGroupeDAge().pipe(catchError(e => of(null))), //15

      ]).subscribe((results) => {
        const userHasRoleRechTous: boolean = AuthenticationUtils.hasRole("ROLE_SA_APPEL_RECH_TOUS");
        ...
        // Ajoute l'option "Sélectionnez..." uniquement si l'utilisateur possède le rôle SA_APPEL_RECH_TOUS
        ...
        this.rechercheFicheAppelCollections.listeNiveauUrgence = this.referencesToInputOptionCollection("niveau-urgence", results[7] as ReferenceDTO[], true);
        ...
        this.rechercheFicheAppelCollections.listeCentreActivite = this.referencesToInputOptionCollection("centre-activite", results[14] as ReferenceDTO[], true);

        // Usager
        this.rechercheFicheAppelCollections.listeLangueUsage = this.referencesToInputOptionCollection("langue-usage", results[8] as ReferenceDTO[], true);
        this.rechercheFicheAppelCollections.listeRegion = this.referencesToInputOptionCollection("region", results[9] as ReferenceDTO[], true);
        this.rechercheFicheAppelCollections.listeSexe = this.referencesToInputOptionCollection("sexe", results[10] as ReferenceDTO[], true);
        this.rechercheFicheAppelCollections.listeGroupeAge = this.referencesToInputOptionCollection("groupe-age", results[15] as ReferenceDTO[], true);
        ...
        // Appelant
        this.rechercheFicheAppelCollections.listeCategorieAppelant = this.referencesToInputOptionCollection("categorie-appelant", results[11] as ReferenceDTO[], true);

        // Note complémentaire
        this.rechercheFicheAppelCollections.listeTypeNote = this.referencesToInputOptionCollection("type-note", results[12] as ReferenceDTO[], true);
        let listeIntervenantNote: ReferenceDTO[] = results[13] as ReferenceDTO[];
        ...
        if (this.rechercheFicheAppelCriteres?.domaine == DOMAINE_SA) {
          ...
          this.rechercheFicheAppelCriteres.domaine = DOMAINE_SA;
  ...
  onMajListeIntervenant(idStOrganisme: number) {
    ...
    // this.subscriptions.add(
    //   this.referencesApiService.getListeIntervenantOrganismeWithRole(idStOrganisme, "SA_APPEL_MODIF", true).subscribe((result: ReferenceDTO[]) => {
    //     const userHasRoleRechTous: boolean = AuthenticationUtils.hasRole("ROLE_SA_APPEL_RECH_TOUS");
    //     // Ajoute l'option "Sélectionnez..." uniquement si l'utilisateur possède le rôle SA_APPEL_RECH_TOUS
  ...
  onRechercherFicheAppel(criteresRechercheFicheAppel: RechercheFicheAppelCriteresDTO): void {
    //console.dir(criteresRechercheFicheAppel);
    this.subscriptions.add(
