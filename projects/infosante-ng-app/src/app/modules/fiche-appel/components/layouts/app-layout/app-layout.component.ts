...
export class AppLayout implements OnInit, OnDestroy {
  ...
  urlSanteApi: string = window["env"].urlSante + '/api';
  ...
  ngOnInit() {
    ...
    this.buildURLAidePageSanteModule(this.getUrlAideFomatted(this.router?.url), this.indexAideEnLigne);
    ...
    this.souscriptions.add(this.router.events.subscribe((event: Event) => {
      if (event instanceof NavigationEnd) {
        if (this.aideEnLigneUrls) {
          ...
          this.souscriptions.add(this.aideEnLigneApiService.getAllAideUrls(this.urlSanteApi).subscribe((result: string[]) => {
            if (result) {
    ...
    if (indexUserMenu >= 0) {
      ...
      if (this.authenticationService.getAuthenticatedUser().NbDaysToPwdExpire <= 10) {
        ...
        this.topRightMenuItems[indexUserMenu].infoBulle = "sigct.sa.f_bandeau.expircpt";
      }
    }
  ...
  private aideEnLigneFactory(currentUrl: string, urls: string[], indexAideEnLigne: any): void {
    ...
    if (validUrl) {
      this.buildURLAidePageSanteModule(this.getUrlAideFomatted(currentUrl), indexAideEnLigne);
    } else {
      if (this.aideEnLigneByDefault) {
        ...
        this.souscriptions.add(this.aideEnLigneApiService.getAideUrlByDefault(this.urlSanteApi).subscribe((result: string) => {
          if (result) {
  ...
  private getUrlAideFomatted(rawUrl: string): string {
    let url: string = "infosante" + this.router?.url;
  ...
  private buildURLAidePageSanteModule(urlFormatted: string, indexAideEnLigne: number): void {
    this.souscriptions.add(
      this.aideEnLigneApiService.buildURLAidePage(this.urlSanteApi, urlFormatted, window["env"].urlAideEnLigne)
   ...
   */
  logout = () => {
    const sub = this.ficheAppelNonTermineService.getNbFicheAppelNonTermine(this.urlSanteApi + "/").subscribe((objJson: any) => {
    ...
    this.souscriptions.add(
      this.consultationContainerStateService.listenSuffixUrlConsultationSubject()
        .subscribe((result: string) => {
          ...
          this.buildURLAidePageSanteModule(currentUrl, this.indexAideEnLigne);
        })
    );
  }
