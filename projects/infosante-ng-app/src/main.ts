...
async function fetchConfigAndLaunchApp() {
  ...
  if (window.location.hostname == "localhost") {
    portailHostUrl = "http://localhost:8080/infosante/portail-host";
  }
  ...
  const envUrl: string = (portailHost?.endsWith("/") ? portailHost : portailHost + "/") + "variables-environnement-angular?appName=infosante"
  response = await fetch(envUrl, {cache: "no-store"});
