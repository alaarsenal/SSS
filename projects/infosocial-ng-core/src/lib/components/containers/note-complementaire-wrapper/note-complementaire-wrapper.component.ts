...
import { EnumRaisonTypeFicheAppel } from 'projects/infosante-ng-core/src/lib/models/raison-type-fiche-appel-enum';
...
import { ReferencesApiService } from '../../../services';
...
/** 100 */
const CODE_LANGUE_DEFAUT: string = "100";

@Component({
  selector: 'so-note-complementaire-wrapper',
...
export class NoteComplementaireWrapperComponent implements OnInit, OnDestroy {
   ...
   */
  chargementNoteComplementaire(): void {
    this.isReadonly = false;

    this.noteComplementaireDTO = new NoteComplementaireDTO();
    this.noteComplementaireDTO.langueAppelCode = CODE_LANGUE_DEFAUT;
    ...
    if (this.appContextStore.state.contexteDetailUsagerPage) {
      ...
      this.abonnements.add(
        this.ficheAppelApiService.getFicheAppel(idFicheAppel).subscribe((ficheAppelDto: FicheAppelSocialDTO) => {
          ...
          if (ficheAppelDto?.usager?.consentementFichesAnterieures) {
            ...
            const msgConsentement: string = this.translateService.instant("sigct.so.f_appel.consultation.suitesintervention.consentementfichesant");
            noteCompl += msgConsentement + "\n\n";
          }

          if (this.isConversionFicheEnNoteCompl) {
            noteCompl += ficheAppelDto.accueil ? ficheAppelDto.accueil + "\n\n" : "";
            if (ficheAppelDto.analyseSituation) {
              noteCompl += this.stripHtmlPipe.transform(ficheAppelDto.analyseSituation);
            }
  ...
  private validerDuree(dto: DureeFicheAppelDTO): boolean {
    this.abonnements.add(this.parametreService.obtenirAdmParameter(this.urlBase, 'DUREE_MAX').subscribe((admParameter: any) => {
      ...
      if (max) {
        ...
        if (dto.dureeCorrigee) {
          ...
          if (dto.dureeCorrigee > dureeAvertissement && dto.dureeCorrigee <= dureeMax) {
            // Durée corrigée : La durée corrigée de la fiche est supérieure à 2 heure. Désirez-vous continuer ?
            this.msgConfirmerDureePopup = this.translateService.instant("so-iu-a30006");
          ...
          if (dureeCalculee > dureeAvertissement && dureeCalculee <= dureeMax) {
            // Durée calculée : La durée calculée de la fiche est supérieure à 2 heure. Désirez-vous continuer ?
            this.msgConfirmerDureePopup = this.translateService.instant("so-iu-a30005");
            this.confirmationDialogService.openAndFocus("confirmer-duree-popup", "confirmer-duree-btn-oui");
            return false;
          }
        }
  ...
  private addNoteAndAttachedFiles() {
    this.abonnements.add(
      this.noteComplementaireService.saveNoteAndboundFiles(this.noteComplementaireDTO, this.eventData.files, this.eventData.detailsFiles, this.idFicheAppel, this.batchFilesToDelete.map(dto => dto.id)).subscribe((result: NoteComplementaireDTO) => {
        if (result) {
   ...
   */
  private calculerDureeFicheConvertie(ficheAppelDto: FicheAppelSocialDTO): Observable<FicheAppelSocialDTO> {
  ...
  private terminerFicheAppelConvertie(idFicheAppel: number): void {
    ...
    this.abonnements.add(
      this.ficheAppelApiService.getFicheAppel(idFicheAppel).pipe(
        concatMap((ficheAppelDto: FicheAppelSocialDTO) => {
          // Prépare la fiche d'appel en cours dans le but de la fermer. 
          // Fiche convertie en note complémentaire de la fiche : {0}
          let accueil: string = ficheAppelDto.accueil ? ficheAppelDto.accueil + "\n\n" : "";
          accueil += this.translateService.instant("sigct.ss.f_appel.consultation.convertirnotecomp.accueil", [this.idFicheAppel]);
          accueil = accueil.substring(0, 1000);

          ficheAppelDto.accueil = accueil;
          ficheAppelDto.codeReferenceTypeFiche = EnumTypeFicheAppel.NONPERT;  // Fiche non pertinente
          ficheAppelDto.referenceRaisonTypeInterventionCode = EnumRaisonTypeFicheAppel.NP50; // Fiche convertie en note complémentaire
          if (!ficheAppelDto.referenceLangueAppelCode) {
        ...
        concatMap((ficheAppelDto: FicheAppelSocialDTO) => {
          return this.usagerService.creerHistoriqueUsagerIdent(ficheAppelDto.usager?.usagerIdentification?.id).pipe(map((usagerIdentHistoDto: UsagerIdentHistoDTO) => {
  ...
  /**
   * Chargement de toutes les listes de valeurs pour réduire les bug d'affichage.
   */
