...
@Component({
  selector: 'so-consultation-fiche-container',
...
export class ConsultationFicheContainerComponent implements OnInit, OnDestroy {
  ...
  showTitleVisualisation: boolean;
  ...
  onWrapperReturnEvent(urlRedirection: UrlTree): void {
    this.stateService.changeState(this._idFicheAppel);
    this.updateWrappersState();
  ...
  /**Actualise l'etat de visualisation de chaque wrapper selon l'etat courant du container */
