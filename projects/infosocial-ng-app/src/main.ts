...
async function fetchConfigAndLaunchApp() {
  ...
  if (window.location.hostname == "localhost") {
    portailHostUrl = 'http://localhost:8080/infosocial/portail-host';
  }
  ...
  const envUrl: string = (portailHost?.endsWith("/") ? portailHost : portailHost + "/") + "variables-environnement-angular?appName=infosocial"
