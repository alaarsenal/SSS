...
export class AccesFicheAppelGuard implements CanActivate {
    ...
    canActivate(route: ActivatedRouteSnapshot, state: RouterStateSnapshot): Observable<boolean | UrlTree> {
        let idFicheAppel: number = route.params["idFicheAppel"];
        if (idFicheAppel) {
            return this.ficheAppelApiService.isFicheAppelAccessible(idFicheAppel).pipe(map((result: boolean) => {
                if (!result) {
