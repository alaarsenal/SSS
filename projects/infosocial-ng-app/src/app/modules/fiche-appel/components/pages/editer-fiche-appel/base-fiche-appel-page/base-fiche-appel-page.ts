...
import { HasAutoSave } from 'projects/infosocial-ng-app/src/app/modules/fiche-appel/services/auto-save-guard.service';
...
export abstract class BaseFicheAppelPage implements OnInit, OnDestroy, HasAutoSave {
  ...
  ngOnInit(): void {
    ...
    this.subscriptions.add(
      this.baseFicheAppelDataService.onFicheAppelActiveChange().subscribe((ficheAppel: FicheAppelSocialDTO) => {
        if (ficheAppel?.id) {
