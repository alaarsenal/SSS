...
export class AccueilFicheAppelPageComponent implements OnInit, OnDestroy {
  ...
  onAjoutConsultationBtnClick(event: MouseEvent) {
    this.subscriptions.add(
  ...
  onAjoutConsultationSaisieDiffereeBtnClick(event: MouseEvent) {
  ...
  private validerFicheAppelNonTerminee(): Observable<boolean> {
    return this.ficheAppelNonTermineService.getNbFicheAppelNonTermine(window["env"].urlInfoSocial + "/api/").pipe(concatMap((objJson: any) => {
      let nb: string = objJson.nbFicheOuverte;
      if (nb && nb !== "0") {
        // so-iu-c70013=Vous avez au moins une intervention en cours dans la liste des fiches non terminées, désirez-vous quand même ajouter une nouvelle intervention manuellement?
        const msgConfirmation: string = this.translateService.instant("so-iu-c70013")
  ...
  private creerAppel(saisieDifferee: boolean) {
    ...
    this.subscriptions.add(
      this.ficheAppelApiService.ajouterAppel(saisieDifferee).subscribe(
        (idAppel: number) => {
          // Accède à la consultation.
  ...
  /**
   * Navigue vers la recherche de fiches d'appel.
   */
  rechercherFicheAppel(): void {
  ...
  */
  onTelechargerFichier(fichier: InformationUtileDTO) {
    ...
    a.target = '_blank';
    a.href = this.fichiersService.getLinktelechargement(Number(fichier.informationsUtilesId), fichier.fichierId);
    a.click();
    a.remove();
  }
