...
@Component({
  selector: 'msss-manager-files-batch',
  templateUrl: './manager-files-batch.component.html',
  styleUrls: ['./manager-files-batch.component.css'],
  providers: [SafePipe]
})
export class ManagerFilesBatchComponent implements OnInit {
  ...
  public filesButchSubject = new BehaviorSubject<UsagerSanterSocialFichierDTO[]>([]);
  public changementDataSource: Observable<any> = this.filesButchSubject.asObservable();

  nbLimitFiles: number = 20;
  ...
  constructor(
    ...
    private alertStore: AlertStore, private safePipe : SafePipe) {
  }
  ...
  public initFichirePourAjouter() {
    this.fichier = new UsagerSanterSocialFichierDTO();
  }

  private abonnementAuxChangementListe() {
    this.abonnement = this.changementDataSource.subscribe(data => {
      if(data.length > this.nbLimitFiles) {
        let msgs: string[] = [];
        msgs.push(this.translateService.instant('ss-iu-e30006'));
  ...
  private initMessageSupression() {
    this.titreSection = this.translateService.instant('sigct.ss.pilotage.info_utile.ajoutmodif.fichier.fichierattache');
    this.msgSupprimer = this.translateService.instant('sa-iu-a00001',[this.titreSection]);
  }
  ...
  getLinktelechargment(fichier: UsagerSanterSocialFichierDTO) {
    ...
    } else if(fichier?.file) {
        const urlToBlob = window.URL.createObjectURL(fichier?.file) 
        return this.safePipe.transform(urlToBlob, 'url');
    }
  }
  ...
  private refreshFichresFromServer() {
    let listEvent = {
      ...
      subject: this.filesButchSubject
    }
  ...
  onBtnAjouterClick() {
   ...
   // newFichier.actionType = ActionType.ADD;
    ...
    let ajouterEvent = {
      ...
      subject: this.filesButchSubject,
      informeAjoute: (data) => {
        this.filesButchSubject.next(data);
        this.fichier = new UsagerSanterSocialFichierDTO();
