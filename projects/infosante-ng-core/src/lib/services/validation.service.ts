...
export class ValidationService {
  ...
  create(dto: ValidationDTO): Observable<ValidationDTO> {
    return this.http.post<ValidationDTO>(this.urlBase + CTX_FICHE_APPEL + CTX_VALIDATION, dto, HTTP_OPTIONS);
  }

  update(dto: ValidationDTO): Observable<ValidationDTO> {
    return this.http.put<ValidationDTO>(this.urlBase + CTX_FICHE_APPEL + CTX_VALIDATION, dto, HTTP_OPTIONS);
  }

  findAllByIdFicheAppel(idFicheAppel: number): Observable<ValidationDTO[]> {
    return this.http.get<ValidationDTO[]>(this.urlBase + CTX_FICHE_APPEL + "/" + idFicheAppel + CTX_VALIDATION);
  }
